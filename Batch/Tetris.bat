...
title - Tetris -
echo/
powershell write-host -fore Darkred - Tetris -
echo/
powershell write-host -fore Red -back Black -NoNewLine "Kodet av:" ; write-host -fore Black -back Black -NoNewLine "/" ; write-host -back Black -fore DarkRed "Arvid" 
echo/
powershell write-host -fore Red -back White -NoNewLine Tetris ; write-host -back White -NoNewLine "/" ; write-host -fore Black -back White spilles med disse tastene:
echo/
echo                                   roter til hoyre
echo roter               roter    beveg        I      beveg 
echo venstre ^<- A S D -^> hoyre    venstre ^<- J K L -^> hoyre
echo              ^|                            ^|
echo              v                            v
echo          soft drop                    hard drop
echo/
powershell write-host -back Black -fore White -NoNewLine "Trykk" ; write-host -back Black -fore Black -NoNewLine "/" ; write-host -back Black -fore Blue -NoNewLine "P" ; write-host -back Black -fore Black -NoNewLine "/" ; write-host -back Black -fore White "for aa pause spillet"
powershell write-host -back Black -fore White -NoNewLine "Trykk" ; write-host -back Black -fore Black -NoNewLine "/" ; write-host -back Black -fore Blue -NoNewLine "N" ; write-host -back Black -fore Black -NoNewLine "/" ; write-host -back Black -fore White "for aa avslutte spillet"
echo/
type TetrisColorBricks.txt
echo/
pause
start Tetris.mp3
cls
...
if %errorlevel% neq 0 (
   echo Konfigurasjons error^^^!
   echo Du maa velge en vindusstorrelse
   echo paa %col% kolonner X %lin% linjer
   pause
   goto :EOF
)
...
for /L %%i in (1,1,%lines%) do set "F%%i=  º%spc%º"
set /A top=lines+1
set "F%top%=  É" & set "F0=  È"
for /L %%i in (1,1,%cols%) do set "F%top%=!F%top%!Í" & set "F0=!F0!Í"
...
%F16x8%           if %%i geq 1 if %%i leq %lines% echo(!F%%i!
               ...
               for /L %%i in (!i!,1,%lines%) do set "F%%i=  º%spc%º"
               call :Delay 95
               cls
               for /L %%i in (%top%,-1,-3) do (
                  echo(!F%%i!
...
:Pause
set "pause=!F%lines%!"
set "F%lines%=  º%spc:          =þþPAUSEþþþ%º"
